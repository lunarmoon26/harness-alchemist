...
- Keep portable workflows in `skills/`; OpenCode runtime code in `src/opencode.ts`; Cordis runtime code in `src/deepseek.ts` with named exports only (no default export).
