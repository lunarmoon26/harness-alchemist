...
- `harness-alchemist.json` is the explicit adaptation boundary for existing monorepos. Keep generated projects strict by default; adapted `pluginRoot` packages choose `runtime: "npm"` (full generated contract) or `runtime: "skills"` (manifests and skills only, any language) and still satisfy the shared skill, manifest, and payload contracts for their mode.
