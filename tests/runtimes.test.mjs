...
test("DeepSeek adapter registers the manifest tool", async () => {
  ...
  const definition = definitions.find((candidate) => candidate.name === "harness_alchemist_run")
...
test(
  ...
  () => {
    const result = spawnSync("python3", [join(root, "skills/harness-alchemist/scripts/main.py")], {
      input: '{"request":"hello"}\n',
      encoding: "utf8",
    })
