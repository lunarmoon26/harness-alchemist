...
export async function validateProject(projectRoot, options = {}) {
  ...
  if (layout.$schema !== undefined && typeof layout.$schema !== "string") {
    errors.push("harness-alchemist.json $schema must be a string")
  }
  if (layout.template !== undefined && !/^v[0-9]+\.[0-9]+\.[0-9]+(-[0-9A-Za-z.-]+)?$/.test(layout.template)) {
    errors.push("harness-alchemist.json template must be a 'vX.Y.Z' version")
  }
  if (layout.generator !== undefined && layout.generator !== "harness-alchemist") {
    errors.push("harness-alchemist.json generator must be 'harness-alchemist'")
  }
  if (
    layout.generatorVersion !== undefined &&
    !/^[0-9]+\.[0-9]+\.[0-9]+(-[0-9A-Za-z.-]+)?$/.test(layout.generatorVersion)
  ) {
    errors.push("harness-alchemist.json generatorVersion must be an 'X.Y.Z' version")
  }
  if (layout.createdAt !== undefined) {
    ...
    if (typeof layout.createdAt !== "string" || Number.isNaN(createdAt.getTime())) {
      errors.push("harness-alchemist.json createdAt must be an ISO 8601 timestamp")
    }
  }
  const runtime = layout.runtime ?? "npm"
  if (!RUNTIME_MODES.includes(runtime)) {
    errors.push(`harness-alchemist.json runtime must be one of: ${RUNTIME_MODES.join(", ")}`)
  }
  ...
  if (opencodeExport !== "." && runtime !== "npm") {
    errors.push("harness-alchemist.json opencodeExport requires runtime 'npm'")
  }
